...
describe("FundMe", function () {
  ...
  describe("cheaperWithdraw", function () {
    ...
    it("Only allows the owner to cheaperWithdraw us", async function () {
      const { owner, fundme, otherAccount } = await loadFixture(
        deployOneYearLockFixture
      );
      const fundmeOther = fundme.connect(otherAccount);
      await expect(fundmeOther.cheaperWithdraw()).to.be.reverted;
    });
  });
});

// npx hardhat test test/fundme_test_unit.js
